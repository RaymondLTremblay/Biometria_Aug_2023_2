...
```{r c05-colorize, eval=TRUE, echo=FALSE}
...
```{r c05-fecha, echo=FALSE}
Sys.Date()
```


```{r c05-hex, echo=FALSE, fig.show="hold", out.width="20%", fig.align="default"}
...
```{r c05-fig-te, echo=FALSE, out.width="45%", out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("Graficos/The_Lady_Drinking_Tea.png")
```

## La dama degustando té

*Era una tarde de verano en Cambridge, Inglaterra, a finales de la década de 1920. Un grupo de profesores universitarios, sus esposas y algunos invitados estaban sentados alrededor de una mesa al aire libre para tomar el té de la tarde. Una de las mujeres insistía en que el té era diferente dependiendo de si el té se vertía en la leche o si la leche se vertía en el té. Las mentes científicas entre los hombres se burlaron de esto como una tontería. ¿Cual podría ser la diferencia? No podían concebir ninguna diferencia en la química de las mezclas que pudieran existir. Un hombre delgado y bajo, con anteojos y una barba de Vandyke que empezaba a ponerse gris, se abalanzó sobre el problema. "Probemos esta propuesta", dijo emocionado. Comenzó a esbozar un experimento en el que a la señora que insistía en que había una diferencia se le presentaría una secuencia de tazas de té, en algunas de las cuales se había vertido la leche en el té y en otras se había vertido el té en la taza de leche.*  Cuento del libro de "The Lady Tasting Tea: How Statistics Revolutionized Science in the Twentieth Century" por David Salsburg.   Traducido con la página de *Google Translate*.

Así comienza el libro de Salsburg (2001), para describir el comportamiento de los científicos cuando están animados de su tema y el deseo de resolver un enigma. Esa persona con barba de Vandyke era Ronald Aylmer Fisher (1890–1962), reconocido como uno de los responsables de la *estadística moderna* y una de las personas más importantes del área en el siglo XX. Sus contribuciones han tenido un impacto en muchas áreas, incluyendo la genética mendeliana y la selección natural.    

## Definición de lo que es la estadística

::: {.definicion}
La **estadística** es la ciencia de recolectar, describir y analizar datos para extraer conclusiones e inferir en presencia de **incertidumbre**.
:::

La estadística es un área de la ciencia de datos que utiliza valores numéricos para evaluar patrones y inferir situaciones futuras. Esta definición es sencilla, pero incluye muchos temas; lo más importante es el componente de utilizar datos, resumirlos en índices o parámetros y usarlos para predecir/inferir el futuro. La palabra estadística, usada en un sentido más o menos similar a su definición de hoy, probablemente se origina del alemán *Statistik*, acuñada por Gottfried Achenwall (1749), donde el término se refería a la descripción de datos del Estado. No fue hasta 1791, con Sir John Sinclair, que el término comenzó a utilizarse para la descripción de conjuntos de datos en general, sin limitarse a los datos de un estado o país.

::: {.historia}
La palabra *estadística* proviene del alemán *Statistik* (Gottfried Achenwall, 1749), originalmente ligada a la descripción del **Estado**. Recién en 1791 Sir John Sinclair la usó para conjuntos de datos en general.
:::      
...
A principios del siglo XVIII, el término estadística servía para designar la colección de información sobre la población y la economía de diferentes regiones o países. En este periodo el objetivo era tabular la información, por ejemplo cuánta gente hay en un área, cuántas vacas hay, cuánta gente es parte de la nobleza.  

Por ejemplo, uno de los primeros estudios para determinar la estadística de una población fue hecho por John Graunt (1662), titulado **Natural and Political Observations Made upon the Bills of Mortality** y puede encontrar el libro original aquí <http://www.neonatology.org/pdf/graunt.pdf>. En el libro expone que 1/3 de los niños muere antes de los 16 años en Londres, Inglaterra. 

Aquí un extracto de una de estas tablas de las causas de enfermedades y mortandad en Londres en el año 1632.  Note que antes de este trabajo estos datos eran muy raros e inexistentes en la mayoría de las ciudades o países. 

Note algunas causas principales de mortandad: "Aged" (628), es decir personas de edad mayor, y "Abortive and Stillborn" (nacidos muertos, 445). Algo que hoy se consideraría raro es morir de "grief", o sea de tristeza (11). 


```{r c05-fig-graunt, echo=FALSE, fig.cap="Tablas de mortalidad de Graunt (1632)", out.width='70%'}
knitr::include_graphics("Graficos/Gaunt_Disease_Casualties_1632.png")
```

El otro componente importante de la estadística fue el desarrollo de las probabilidades en el siglo 17 y 18.  A la gente le gustaba (como hoy en día) hacer apuestas y jugar a las cartas por dinero. Muchos trataban de entender las probabilidades de ganar en estos juegos de azar para aumentar sus posibilidades.  
...
En 1939, la nombraron profesora asistente en Iowa State College y, en 1940, dirigió el primer departamento de Estadística Experimental en la Escuela de Agricultura de la Universidad de Carolina del Norte. Esto la convirtió en la primera mujer directora del recinto. Además, Cox fue la primera mujer jefa del Instituto de Estadística de la Universidad de Carolina del Norte en 1944. Un año más tarde, participó como editora principal de la revista **Biometrics** durante 10 años, y fundó, en 1947, la **International Biometric Society**. Además, se convirtió en presidenta de la ASA (*American Statistical Association*) en 1956.

```{r c05-fig-farm, echo=FALSE, fig.align='center'}
