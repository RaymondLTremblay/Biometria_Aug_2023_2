...
Añadir los siguientes paquetes, Nota la función de **pacman** para instalar los paquetes.  Esta función hace que si le falta un paquete en la lista lo va a instalar sin hacer tener que hacerlo uno por uno. Si quiere hacerlo separado remueve el **#** antes de **install.packages** 
...
library(car) # Paquete para acompañar laregresión lineal
library(ggplot2)  # Visualización de datos
library(pastecs) # Paquete para análisis estadístico
library(psych)   # Paquete para análisis estadístico en psicología
...
La ética en estadística se basa en comprobar y cumplir con los supuestos de las pruebas estadísticas que se usan. La gran mayoría de las pruebas tradicionales asumen que los datos cumplen con algunos supuestos; algunos típicos son: 

1. Distribución normal
2. Igualdad de varianza = Homogeneidad de Varianza
3. Simetría de las distribuciones
4. Datos recolectados al azar

::: {.supuestos}
Las pruebas paramétricas más comunes (prueba t, ANOVA, regresión) asumen: **normalidad** de los datos o de los residuales, **homogeneidad de varianza** entre grupos, e **independencia** de las observaciones. Si no se cumplen, conviene transformar los datos o usar pruebas no paramétricas.
:::
 

Cuando no se cumplen los supuestos, aumentamos la probabilidad de cometer un error de tipo I (rechazar la hipótesis nula cuando en realidad es cierta) o un error de tipo II (no rechazar la hipótesis nula cuando en realidad es falsa).

Por consecuencia, es **primordial** que cada investigador evalúe los supuestos de las pruebas que usa y, si no se cumplen, busque métodos alternos. Por ejemplo, se pueden utilizar métodos no paramétricos, estadística bayesiana o robusta como alternativas.  

En este módulo se explica cómo asegurar que los datos cumplan con los supuestos básicos de las pruebas paramétricas, que incluyen la prueba t, ANOVA, entre otras.  

Note que el énfasis de este módulo es evaluar los supuestos con herramientas visuales; también existen pruebas estadísticas disponibles para cada uno de estos supuestos. Vea la sección de estadística para más detalles sobre esas alternativas.  
...
```{r c10-4, rexample=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r c10-5, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r c10-6, rexample=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
dipodium2=dipodium %>% 
...
A continuación se ilustra otro enfoque para evaluar los residuales. En este caso, para detectar valores sesgados (outliers). Para visualizar los residuales se usan los residuales estandarizados (divididos por la desviación estándar). Valores mayores de 3 se considerarían atípicos. Esta alternativa es apropiada para detectar valores atípicos.  Note en la figura que se usa otra vez el modelo **modelflower** y los valores calculados en el modelo **.fitted** y los residuales de student con **.stdresid**. Note ahora también que todos los valores en el eje de **Y** son mayores de cero.

```{r c10-8, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r c10-9, echo=TRUE, message=FALSE, warning=FALSE}
...
Los valores que parecen atípicos pueden ser normales de la población estudiada; en esos casos, removerlos podría llevar a describir los resultados de forma incorrecta. Primero hay que evaluar las siguientes posibles causas. Si alguna de ellas está presente, hay que corregir los valores o removerlos de la hoja de análisis.  


* los valores recolectados son erróneos.
* los valores fueron introducidos incorrectamente en la hoja de datos.
* los valores recolectados tienen diferentes dimensiones; por ejemplo, algunos en cm y otros en mm para la misma variable.

::: {.advertencia}
No elimines valores atípicos automáticamente. Un valor extremo puede ser real y biológicamente válido; removerlo sin justificación sesga los resultados. Elimínalo solo si identificas una causa clara (error de medición o de captura).
::: 
...
Hay muchas pruebas estadísticas que asumen que los datos provienen de una distribución normal. Pero no todas las pruebas tienen que satisfacer ese supuesto.  Por ejemplo, la prueba de t o el ANOVA asumen distribución normal.  Por el contrario, las pruebas no paramétricas como Wilcoxon, Mann-Whitney, Kruskall Wallis y otras no asumen ese supuesto.

Una alternativa tradicional para comprobar la normalidad de los datos es mirar el gráfico de QQ; o sea, una visualización para comparar las probabilidades de dos variables en las que se grafican los cuantiles (recordemos que **Q** viene de *quantile*, cuantil). Si las distribuciones de las dos variables son similares, los puntos aparecerán nítidamente en el gráfico de QQ sobre la línea de **X-Y**. Eso se demostrará a continuación.
...
```{r c10-10, echo=TRUE, message=FALSE, warning=FALSE}
...
::: {.funcion}
**`geom_qq()` + `geom_qq_line()`** — gráfico Q-Q en ggplot2.

- **Qué hacen:** dibujan los cuantiles observados frente a los teóricos (`geom_qq`) y la línea de referencia (`geom_qq_line`).
- **Cómo leer:** si los puntos siguen la línea, los datos son compatibles con la distribución normal.
:::

A pesar de lo fácil de este enfoque, apreciar si los datos quedan nítidamente alineados sobre una línea no es tan evidente en algunos casos.  Por lo tanto, en la siguiente versión del gráfico de QQ producida con la función **geom_qq** y **geom_qq_line** se le añade una línea para mostrar dónde debería estar localizada la gran mayoría de los datos si estos tuvieran una distribución normal. Ahora vemos que claramente muchos de los datos no yacen en la línea.  Evidentemente, el trazar la línea teórica ayuda a visualizar la conclusión; en este caso, se podría concluir que el consumo de comida por este pájaro no sigue una distribución normal `r colorize("la linea azul", "blue")`.  En el segundo ejemplo se crea un conjunto de datos con distribución normal y, en ese caso, la gran mayoría de los datos se solapan con la línea, `r colorize("la línea roja", "red")`

::: {.interpretacion}
En un gráfico Q-Q, si los datos siguen una distribución normal los puntos caen sobre la línea diagonal. Desviaciones sistemáticas en los extremos (colas) indican que los datos **no** son normales.
:::  

```{r c10-11, echo=TRUE, message=FALSE, warning=FALSE}
...
::: {.funcion}
**`ggpairs(datos)`** (paquete **GGally**) — matriz de correlaciones.

- **Qué hace:** muestra en una sola figura los diagramas de dispersión, las distribuciones y los coeficientes de correlación de todos los pares de variables.
- **Uso aquí:** detectar **colinealidad** (predictoras muy correlacionadas entre sí) antes de ajustar un modelo.
:::
...
A continuación exploraremos diferentes alternativas para llevar a cabo este tipo de análisis utilizando otra vez los datos de la orquídea *Dipodium*.  En el primer ejemplo, se utiliza la función **cor** para evaluar solo una relación: la correlación entre la distancia de la planta a un árbol y la altura de la planta, dejando otras variables a un lado.  Como deseamos el estimado de la correlación solamente, utilizamos un par de variables a la vez para evaluar la correlación entre la distancia de la orquídea al árbol y la altura de la inflorescencia (mostrado más adelante).  Concluimos que a mayor distancia del árbol, menor es la altura de la inflorescencia; este análisis arroja un valor de correlación de -0.095, como se muestra en el ejemplo.

```{r c10-15, rexample=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r c10-16, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r c10-17, echo=TRUE, message=FALSE, warning=FALSE}
kable(signif(cor(dipodium[,c(3, 8, 9)],
 method="kendall",
 use="pairwise.complete.obs"),2), 
 caption =
...
```{r c10-18, echo=TRUE, message=FALSE, warning=FALSE}
