...
::: {.historia}
La distribución *t* fue publicada en 1908 por **William Sealy Gosset**, un químico y estadístico que trabajaba en la cervecería **Guinness** en Dublín. Como Guinness prohibía a sus empleados publicar, Gosset firmó con el seudónimo **"Student"**; por eso hoy se le llama la *t* de Student. La desarrolló para trabajar con las **muestras pequeñas** típicas del control de calidad de la cerveza.
:::
...
::: {.funcion}
**`qt(p, df)`** — cuantil de la distribución *t* (el "valor crítico").

- **Qué hace:** devuelve el valor de *t* que deja una probabilidad `p` acumulada a su izquierda, con `df` grados de libertad.
- **Uso aquí:** `qt(0.025, 19)` da el valor crítico para una prueba de dos colas al 95% con 19 gl (por eso se usa 0.025 y su valor absoluto).
- **Familia:** `dt` (densidad), `pt` (probabilidad acumulada) y `rt` (datos aleatorios).
:::  
...
::: {.funcion}
**`t.test(x, mu = ..., alternative = ...)`** — prueba de *t* de una muestra.

- **Qué hace:** compara el promedio de la variable `x` con un valor de referencia `mu` (el de la hipótesis nula).
- **Argumentos clave:** `mu` (valor a probar), `alternative` (`"two.sided"`, `"less"` o `"greater"`) y `conf.level` (nivel de confianza; 0.95 por defecto).
- **Cómo leer la salida:** el valor-p indica si se rechaza la Ho; el intervalo de confianza da el rango probable del promedio real.
:::
