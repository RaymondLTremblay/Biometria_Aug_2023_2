...
- **Qué hace:** ajusta un modelo cuando la respuesta no es normal; con `family = binomial()` realiza una **regresión logística** para una `y` binaria (0/1).
- **Diferencia con `lm()`:** modela el *logit* de la probabilidad, no la respuesta directamente.
- **Cómo leer la salida:** los coeficientes están en escala logit; un coeficiente positivo aumenta la probabilidad de "1" al crecer `x`. Revisa el valor-p de cada uno.
:::
...
- **Argumento clave:** `method.args = list(family = binomial)` le indica que use un GLM binomial (en lugar de una recta).
- **Opción:** `se = TRUE` muestra la banda del intervalo de confianza; `se = FALSE` la oculta.
:::
