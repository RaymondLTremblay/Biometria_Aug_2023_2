...
::: {.historia}
Esta prueba tiene dos orígenes. **Frank Wilcoxon** —químico y estadístico— publicó las pruebas de rangos en 1945. Dos años después, **Henry Mann** y **Donald Whitney** (1947) desarrollaron una versión equivalente, la *U*. Por eso hoy se conoce con ambos nombres.
:::
...
::: {.funcion}
**`wilcox.test(y ~ grupo, data)`** — prueba de Wilcoxon / Mann-Whitney.

- **Qué hace:** compara dos grupos usando los **rangos** de los datos, sin asumir normalidad. Con `paired = TRUE` hace la versión para datos pareados (rangos con signo).
- **Argumentos útiles:** `exact = FALSE` (necesario cuando hay empates) y `conf.int = TRUE` (añade el intervalo de confianza).
- **Cómo leer:** el valor-p indica si las distribuciones (ubicación/mediana) difieren.
:::
...
::: {.historia}
La prueba fue publicada en 1952 por **William Kruskal** y **W. Allen Wallis**. Extiende la idea de Wilcoxon a tres o más grupos y es la alternativa no paramétrica al ANOVA de una vía.
:::
...
::: {.funcion}
**`kruskal.test(y ~ grupo, data)`** — prueba de Kruskal-Wallis.

- **Qué hace:** compara la distribución (mediana/rangos) de `y` entre **tres o más** grupos, sin asumir normalidad ni igualdad de varianza.
- **Cómo leer:** el valor-p indica si al menos un grupo difiere; si es < 0.05, se hace una prueba post-hoc (Nemenyi) para ver cuáles.
:::
...
::: {.historia}
Las pruebas de **permutación** (calcular la significancia reordenando los datos de todas las formas posibles) se remontan a **R. A. Fisher** en los años 1930. Cuando hay demasiadas permutaciones para enumerarlas, se aproximan con simulación **Monte Carlo**, método bautizado así por **Stanisław Ulam**, **John von Neumann** y **Nicholas Metropolis** en los años 1940, en la era de las primeras computadoras.
:::

::: {.funcion}
**`kruskal_test(y ~ grupo, data, distribution = approximate(nresample = ...))`** (paquete **coin**).

- **Qué hace:** versión de Kruskal-Wallis que estima el valor-p por **simulación** (permutaciones Monte Carlo) en lugar de una fórmula teórica.
- **Argumento clave:** `nresample` es el número de permutaciones (aquí 10 000): más permutaciones = estimación más precisa.
:::
...
::: {.historia}
Esta prueba de comparaciones múltiples fue desarrollada por **Peter Nemenyi** en 1963 como el equivalente no paramétrico de la prueba de Tukey.
:::
...
::: {.funcion}
**`kwAllPairsNemenyiTest(y ~ grupo, data)`** (paquete **PMCMRplus**) — post-hoc de Nemenyi.

- **Qué hace:** tras un Kruskal-Wallis significativo, compara todos los pares de grupos ajustando por comparaciones múltiples.
- **Cómo leer:** la salida es una matriz de valores-p; cada par con p < 0.05 difiere significativamente.
:::
