...
**Presentación de temas y discusion**: M, J at 1:30-2:50pm (80 mins) 
...
```{r intro 2, echo=FALSE, fig.cap="El proceso de Investigación", out.width = '60%'}
...
```{r intro 3, echo=FALSE, fig.cap="El proceso de Investigación", out.width = '70%'}
...
```{r pop2, echo=FALSE, fig.show = "hold", out.width = "100%", fig.align = "default"}
...
<!--chapter:end:04-Inferencias_Hipotesis.Rmd-->
...
```{r H1, echo=FALSE, fig.align='center', fig.width = 50, echo=FALSE}
...
### Revista *Biometrics*
```{r H2, echo=FALSE, out.width= "45%", out.extra='style="float:right; padding:10px"'}
...
```{r, central_tendency7, warning=FALSE, message=FALSE}
df=rpois(5000, 100)
df
df1=data.frame(df)
head(df1)
...
```{r central_tendency8, warning=FALSE, message=FALSE}
...
```{r central_tendency9, warning=FALSE, message=FALSE}
...
```{r central_tendency10, warning=FALSE, message=FALSE}
library(ggplot2)
...
```{r central_tendency11, warning=FALSE, message=FALSE}
library(ggplot2)
b=ggplot(df1, aes(df))+
  geom_histogram(fill="orange", colour="white", binwidth = 1)+
  geom_vline(aes(xintercept = pro), colour="red")+
...
```{r central_tendency12, warning=FALSE, message=FALSE }
...
```{r central_tendency13, warning=FALSE, message=FALSE}
library(tidyverse)
...
```{r central_tendency14, warning=FALSE, message=FALSE}
mea=mean(dfb1$dfb1)
...
```{r central_tendency15, warning=FALSE, message=FALSE}
...
```{r central_tendency16, warning=FALSE, message=FALSE}
...
```{r, dispersion1, eval=TRUE, echo=FALSE}
...
```{r dispersion4, echo=FALSE, fig.align='center', out.width = '60%'}
...
### Cual es el rango de la Edad de los padres de los Estudiantes cuando nacieron sus hijos
...
```{r dispersion21, echo=FALSE, fig.align='center', out.width = '60%'}
...
```{r dispersion22,  echo=FALSE, fig.align='center', out.width = '60%'}
knitr::include_graphics("Graficos/CI_a1.png")
```


```{r dispersion23, fig.align='center', warning=TRUE}
...
```{r dispersion24,  echo=FALSE, fig.align='center', out.width = '60%'}
knitr::include_graphics("Graficos/CI_b.png")
```


```{r dispersion 25, eval=FALSE, include=FALSE}
...
```{r dispersion26, message=FALSE, warnings=FALSE, results='asis'}
require(pander)
...
```{r, dispersion27, message=FALSE, warnings=FALSE}
...
```{r dispersion30, message=FALSE, warning=FALSE}
library(readr)
library(gt)
...
```{r dispersion33, echo=FALSE, eval=FALSE}
...
```{r dispersion34, echo=FALSE, eval=FALSE}
...
```{r desc19, echo=FALSE, eval=FALSE}
...
```{r desc20, echo=FALSE, eval=FALSE}
...
```{r desc21, echo=FALSE, eval=FALSE}
library(ggversa)
head(Camas_Hospital)
...
```{r desc22, echo=FALSE, eval=FALSE}
...
```{r desc23, echo=FALSE, eval=FALSE}
...
```{r ggplot2, eval=TRUE, echo=FALSE}
colorize <- function(x, color) {
  if (knitr::is_latex_output()) {
    sprintf("\\textcolor{%s}{%s}", color, x)
  } else if (knitr::is_html_output()) {
...
```{r supuesto1, rexample=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supesto2, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supuesto3, rexample=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
dipodium2=dipodium %>% 
...
```{r supesto5, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supesto6, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supesto7, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supesto8, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supuesto12, rexample=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supuesto13, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supesto14, echo=TRUE, message=FALSE, warning=FALSE}
kable(signif(cor(dipodium[,c(3, 8, 9)],
 method="kendall",
 use="pairwise.complete.obs"),2), 
 caption =
...
```{r supesto15, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r supuesto16, warning=FALSE, message=FALSE}
...
```{r freq6, echo=FALSE, out.width= "25%", out.extra='style="float:right; padding:10px"'}
...
```{r freq15, echo=FALSE, out.width= "60%", out.extra='style="float:right; padding:10px"'}
...
```{r freq16, eval=FALSE, echo=FALSE}
#Crear la tabla de contingencia
...
```{r norm6, warning=FALSE, message=FALSE}
...
```{r onesamp12, echo=FALSE, fig.cap="Su promedio incluye el promedio del universo?", out.width = '70%'}
...
```{r onesamp13, eval=FALSE, echo=FALSE}
...
```{r onesamp14, eval=FALSE, echo=FALSE}
...
```{r tpare9, echo=FALSE, fig.cap="*Lepanthes eltoroensis*",fig.align="center", out.width = '40%'}
...
```{r tpare10, eval=FALSE, echo=FALSE}
...
```{r tind2, attr.source='.numberLines'}
genero=c("m","f","f","f",
    "f","f","f","f","f",
    "m","f","m","f","f",
    "m","m","f","f","f",
...
Unir las tres lisatas de datos en un data frame con la función de "data.frame( )"
...
Hacer un  gráfico de los datos.  Si calculamos el promedio de todos los valores de "Y" y lo visualizamos, vemos que muchos de los valores del municipio "Bayamón" son muy por debajo los valores de la linea y muchos de los valores del municipio "U
tuado" son muy por encima.    
...
```{r tind7}
model=t.test(dist~muni,data=viajes)
model
```
...
model2=t.test(dist~gender, data=df)
model2
```
...
Cuando los datos no cumple con distribución igualdad de varianza se dice que los datos heteroscedastico.  
...
```{r tind19, echo=FALSE, eval=FALSE}
df1=rnorm(100)
...
```{r anova18, eval=FALSE, echo=FALSE}
...
```{r anova19, eval=FALSE,echo=FALSE}
#Superhero
...
```{r anova20, eval=FALSE,echo=FALSE}
library(ggplot2)
...
```{r anva21, eval=FALSE, echo=FALSE}
...
```{r anova22, eval=FALSE, echo=FALSE}
...
```{r anova23, eval=FALSE, echo=FALSE}
# Hacer la prueba de ANOVA con aov()
...
```{r anova24, eval=FALSE, echo=FALSE}
...
```{r anova25, eval=FALSE, echo=FALSE}
# Hacer la prueba de post-hoc si es necesario!!!!!
...
```{r reg14, eval=FALSE, include=FALSE}
model2=lm(Altura_cm~Peso_kg, data=Alturas_Humanos)
summary(model2)
```

```{r reg15, eval=FALSE, include=FALSE}
plot(model2)
```


```{r reg 16, eval=FALSE, include=FALSE}
...
```{r reg17, eval=FALSE, include=FALSE}
library(ggversa)
library(ggplot2)
```


```{r reg18, eval=FALSE, include=FALSE}
...
```{r reg19, eval=FALSE, include=FALSE}
...
```{r reg20, eval=FALSE, include=FALSE}
plot(modelD)
```


```{r reg21, eval=FALSE, include=FALSE}
ggplot(dipodium, aes(Height_Inflo, Number_of_Flowers))+
geom_point()+
...
```{r reglog1, echo=FALSE, fig.cap="**La especie** *Brasavolla cucullata* **en la isla de San Eustaquio**",fig.show="hold", out.width = "50%"}
...
```{r akaike10, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(huxtable)
...
```{r akaike12, message=FALSE, warning=FALSE}
